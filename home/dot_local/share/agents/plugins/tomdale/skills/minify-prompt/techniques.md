...
you" -> just state the action, "please note that" -> delete, "you should
always" -> always, "keep in mind that" -> delete, "the reason for this is" ->
because, "due to the fact that" -> because, "in the event that" -> if, "at
...
version and delete the others. Don't add "as noted above/below", just state the
fact once.
...
Use key: value for definitions. Use parenthetical annotations for metadata. The
agent doesn't need visual hierarchy cues, it processes the full text
sequentially.
